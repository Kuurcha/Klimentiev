...
int main() {
    ...
    fprintf(stdout,BLUE "\nMulti:" NONE " Start multi proc"); fflush(stdout);
    ...
    while(1){
        ...
        fprintf(stdout, BLUE"\nMulti:" NONE " Waiting data..."); fflush(stdout);
        if (semop(s1, &b1, 1)<0){
            printf("Multi Can\'t wait for condition\n");
            exit(-1);
        } // блокируем текущий процесс пока семафор не станет равным 2
        fprintf(stdout, BLUE"\nMulti:" NONE " Start calc"); fflush(stdout);
        ...
        fprintf(stdout,BLUE "\nMulti:" NONE " Коеф1: %f", coef1); fflush(stdout);
        fprintf(stdout,BLUE "\nMulti:" NONE " Коеф2: %f", coef2); fflush(stdout);
        ...
        if (semctl(s1, 0, SETVAL, u )){
            printf(BLUE "Multi: " NONE " Can\'t set val\n");
            exit(-1);
        }

        fprintf(stdout, BLUE "\nMulti:" NONE " Finish calc"); fflush(stdout);
