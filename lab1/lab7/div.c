...
int main() {
    ...
    fprintf(stdout,BROWN "\nDiv:" NONE " Start div proc"); fflush(stdout);
    ...
    while(1){
        ...
        fprintf(stdout, BROWN"\nDiv:" NONE " Waiting data..."); fflush(stdout);
        if (semop(s1, &b1, 1)<0){
            printf("Div Can\'t wait for condition\n");
            exit(-1);
        } // блокируем текущий процесс пока семафор не станет равным 2
        fprintf(stdout, BROWN"\nDiv:" NONE " Start calc"); fflush(stdout);
        ...
        fprintf(stdout,BROWN "\nDiv:" NONE " Коеф1: %f", coef1); fflush(stdout);
        fprintf(stdout,BROWN "\nDiv:" NONE " Коеф2: %f", coef2); fflush(stdout);
        ...
        if (semctl(s1, 0, SETVAL, u )){
            printf(BROWN "Div: " NONE " Can\'t set val\n");
            exit(-1);
        }

        fprintf(stdout, BROWN "\nDiv:" NONE " Finish calc"); fflush(stdout);
