...
int main() {
    ...
    fprintf(stdout,YELLOW "\nPlus:" NONE " Start plus proc"); fflush(stdout);
    ...
    while(1){
        ...
        fprintf(stdout,YELLOW "\nPlus:" NONE " Коеф1: %f", coef1); fflush(stdout);
        fprintf(stdout,YELLOW "\nPlus:" NONE " Коеф2: %f", coef2); fflush(stdout);
        ...
        if (semctl(s1, 0, SETVAL, u )){
            printf(YELLOW "Plus: " NONE " Can\'t set val\n");
            exit(-1);
        }

        fprintf(stdout, YELLOW "\nPlus:" NONE " Finish calc"); fflush(stdout);
